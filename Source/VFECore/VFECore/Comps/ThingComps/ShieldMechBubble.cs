		...
		private Material BubbleMat
        {
			get
            {
				if (bubbleMat is null)
                {
                    ...
                    {
						bubbleMat = MaterialPool.MatFrom("Other/ShieldBubble", ShaderDatabase.Transparent);
					}
					else
                    {
						bubbleMat = MaterialPool.MatFrom(Props.shieldTexPath, ShaderDatabase.Transparent);
					}
				}
				return bubbleMat;
            }
