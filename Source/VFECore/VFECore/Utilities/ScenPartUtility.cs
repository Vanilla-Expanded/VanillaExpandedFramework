...
namespace VFECore
{
    public static class ScenPartUtility
    {
        ...
        public static Dictionary<FactionDef, IntRange> cachedFactionDefAffectedByForcedFactionGoodwill;

        public static void SetCache()
        {
            cachedFactionPermanentEnemyFlags = new Dictionary<FactionDef, bool>();
            cachedFactionDefAffectedByForcedFactionGoodwill = new Dictionary<FactionDef, IntRange>();
        ...
        public static void FinaliseFactionGoodwillCharacteristics(FactionDef faction)
        {
            ...
            for (int i = goodwillScenParts.Count - 1; i >= 0; i--)
            {
                ...
                if (curScenPart.AffectsFaction(faction))
                {
                    ...
                    if (curScenPart.affectStartingGoodwill)
                    {
                        cachedFactionDefAffectedByForcedFactionGoodwill.Add(faction, curScenPart.startingGoodwillRange);
                    }
                    return;
                }
            }
