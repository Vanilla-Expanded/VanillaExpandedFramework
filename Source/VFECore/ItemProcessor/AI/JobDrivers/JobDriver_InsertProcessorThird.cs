...
{
    public class JobDriver_InsertProcessorThird : JobDriver
    {
        ...
        protected override IEnumerable<Toil> MakeNewToils()
        {
            ...
            yield return Toils_General.DoAtomic(delegate
            {
                ...
                if (building_processor.ExpectedAmountThirdIngredient != 0)
                {
                    this.job.count = building_processor.ExpectedAmountThirdIngredient - building_processor.CurrentAmountThirdIngredient;
                }
                else this.job.count = 1;
            ...
            yield return new Toil
            {
                initAction = delegate
                {
                    ...
                    if (building_processor.processorStage != ProcessorStage.Inactive)
                    {
                        ...
                        building_processor.CurrentAmountThirdIngredient += this.job.targetB.Thing.stackCount;
                        if (building_processor.ExpectedAmountThirdIngredient != 0)
                        {
                            if (building_processor.CurrentAmountThirdIngredient >= building_processor.ExpectedAmountThirdIngredient)
                            { building_processor.thirdIngredientComplete = true; }
                        }
                        building_processor.TryAcceptThird(this.job.targetB.Thing, 0, true);
