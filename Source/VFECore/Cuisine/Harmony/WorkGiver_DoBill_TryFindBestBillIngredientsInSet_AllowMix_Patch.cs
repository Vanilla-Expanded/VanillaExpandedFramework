        ...
        public static IEnumerable<CodeInstruction> Transpiler(IEnumerable<CodeInstruction> instructions, ILGenerator ilg)
        {
            ...
            for (int i = 0; i < instructionList.Count; i++)
            {
                ...
                if(instruction.Calls(isFixedInfo))
                {
                    ...
                    yield return CodeInstruction.LoadField(typeof(WorkGiver_DoBill_TryFindBestBillIngredientsInSet_AllowMix_Patch), nameof(adjust));
                    yield return new CodeInstruction(OpCodes.Brfalse, skip);
                    yield return CodeInstruction.LoadField(typeof(WorkGiver_DoBill_TryFindBestBillIngredientsInSet_AllowMix_Patch), nameof(alreadyUsed));
