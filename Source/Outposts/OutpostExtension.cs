...
namespace Outposts
{
    public class OutpostExtension : DefModExtension
    {
        ...
        public List<SkillDef> RelevantSkills =>
            new HashSet<SkillDef>(RequiredSkills.Select(rq => rq.Skill).Concat(ResultOptions.SelectMany(ro => ro.AmountsPerSkills.Select(aps => aps.Skill)))).ToList();
    }
    ...
    public class ResultOption
    {
        ...
        public int Amount(List<Pawn> pawns) => AmountPerPawn * pawns.Count + AmountsPerSkills.Sum(x => x.Amount(pawns));
