...
namespace MVCF.Utilities
{
    ...
    {
        public static IEnumerable<VerbComp_Reloadable> AllReloadComps(this Pawn p) => p.Manager().ManagedVerbs.SelectMany(mv => mv.AllComps).OfType<VerbComp_Reloadable>();
            ...
            foreach (var comp in twc.AllComps)
                switch (comp)
                {
                    case CompEquippable eq:
                        foreach (var verbComp in eq.AllVerbs.SelectMany(verb => verb.Managed().AllComps).OfType<VerbComp_Reloadable>())
                            yield return verbComp;
                        break;
                    case Comp_VerbGiver giver:
                        foreach (var reloadable in giver.VerbTracker.AllVerbs.SelectMany(verb => verb.Managed().AllComps).OfType<VerbComp_Reloadable>())
