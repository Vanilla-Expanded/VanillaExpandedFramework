        ...
        public static List<CustomVector> Run(int maxTries, int Width, int Height, Random r, CustomVector[][] grid)
        {
            ...
            while (active.Count > 0)
            {
                ...
                for (int tries = 1; tries <= maxTries; tries++)
                {
                    ...
                    int new_radius = r.Next(GenOption.radius, (int)(1.5f * GenOption.radius));
                    ...
                    if (IsInBound(pnew, Width, Height, GenOption.radius) && InsideCircles(pnew, GenOption.radius, points))
                    {
                        points.Add(pnew);
                        active.Add(pnew);
                        break;
