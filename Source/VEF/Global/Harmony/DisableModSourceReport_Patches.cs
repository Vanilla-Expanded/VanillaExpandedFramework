    ...
    public static class VanillaExpandedFramework_ResearchProjectDef_GetTip_Patch
    {
        public static IEnumerable<CodeInstruction> Transpiler(IEnumerable<CodeInstruction> codeInstructions)
        {
            ...
            for (int i = 0; i < codes.Count; i++)
            {
                ...
                {
                    yield return new CodeInstruction(OpCodes.Call, AccessTools.Method(typeof(VanillaExpandedFramework_ResearchProjectDef_GetTip_Patch), nameof(ShouldShow)));
    ...
    public static class VanillaExpandedFramework_BackstoryDef_FullDescriptionFor_Patch
    {
        public static IEnumerable<CodeInstruction> Transpiler(IEnumerable<CodeInstruction> codeInstructions)
        {
            ...
            for (int i = 0; i < codes.Count; i++)
            {
                ...
                if (codes[i].opcode == OpCodes.Brtrue_S && codes[i - 1].Calls(get_IsCoreModInfo))
                {
                    yield return new CodeInstruction(OpCodes.Call, AccessTools.Method(typeof(VanillaExpandedFramework_BackstoryDef_FullDescriptionFor_Patch), nameof(ShouldShow)));
                    yield return new CodeInstruction(OpCodes.Brfalse_S, codes[i].operand);
                }
            }
        }
