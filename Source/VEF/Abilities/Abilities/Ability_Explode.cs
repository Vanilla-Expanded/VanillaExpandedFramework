            ...
            if (def.GetModExtension<AbilityExtension_Explosion>() is AbilityExtension_Explosion ext)
            {
                foreach (GlobalTargetInfo target in targets)
                {
                    GenExplosion.DoExplosion(ext.onCaster ? pawn.Position : target.Cell, pawn.Map, ext.explosionRadius, ext.explosionDamageDef, pawn,
                                             ...
                                             ext.postExplosionSpawnChance, ext.postExplosionSpawnThingCount, ext.postExplosionGasType, ext.postExplosionGasRadiusOverride, ext.postExplosionGasAmount, ext.applyDamageToExplosionCellsNeighbors, ext.preExplosionSpawnThingDef,
