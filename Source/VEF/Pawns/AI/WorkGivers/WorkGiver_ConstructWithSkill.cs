...
namespace VEF.Pawns
{
            ...
            foreach (CodeInstruction instruction in codeInstructions)
            {
                ...
                if (!patched && instruction.opcode == OpCodes.Stloc_3)
                {
                    ...
                    yield return new CodeInstruction(OpCodes.Call,
                        AccessTools.Method(typeof(VanillaExpandedFramework_Designator_Build_DrawPanelReadout_Patch), "DrawSkillRequirementIfNeeded"));
                }
            }
        }
    ...
    public static class VanillaExpandedFramework_Frame_CompleteConstruction_Patch
    {
        public static IEnumerable<CodeInstruction> Transpiler(IEnumerable<CodeInstruction> codeInstructions)
        {
            ...
            var interceptSkillInfo = AccessTools.Method(typeof(VanillaExpandedFramework_Frame_CompleteConstruction_Patch), "InterceptSkill");
            foreach (CodeInstruction instruction in codeInstructions)
            {
                yield return instruction;
    ...
    public static class VanillaExpandedFramework_JobDriver_ConstructFinishFrame_MakeNewToils_TickAction_Patch
    {
        ...
        {
            var constructionField = AccessTools.Field(typeof(SkillDefOf), nameof(SkillDefOf.Construction));
            var interceptSkillInfo = AccessTools.Method(typeof(VanillaExpandedFramework_JobDriver_ConstructFinishFrame_MakeNewToils_TickAction_Patch),
                "InterceptSkill");
            var shouldSkipCheckInfo = AccessTools.Method(typeof(VanillaExpandedFramework_JobDriver_ConstructFinishFrame_MakeNewToils_TickAction_Patch),
    "ShouldSkipCheck");
