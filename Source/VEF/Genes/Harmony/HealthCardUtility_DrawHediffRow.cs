        ...
        public static IEnumerable<CodeInstruction> Transpiler(IEnumerable<CodeInstruction> codeInstructions, ILGenerator ilg)
        {
            ...
            for (var i = 0; i < codes.Count; i++)
            {
                ...
                if (codes[i].opcode == OpCodes.Stfld && codes[i - 1].LoadsField(bleedingIconStaticField))
                {
                    ...
                    yield return new CodeInstruction(OpCodes.Call, AccessTools.Method(typeof(VanillaExpandedFramework_HealthCardUtility_DrawHediffRow_Patch), 
                        nameof(VanillaExpandedFramework_HealthCardUtility_DrawHediffRow_Patch.HasBloodIconChangingGene), new Type[] { typeof(Pawn) }));
                    yield return new CodeInstruction(OpCodes.Brfalse_S, label);
                    yield return new CodeInstruction(OpCodes.Ldloc_S, 12);
                    yield return new CodeInstruction(OpCodes.Call, AccessTools.Method(typeof(VanillaExpandedFramework_HealthCardUtility_DrawHediffRow_Patch), "ChangeIconForThisPawn", null, null));
