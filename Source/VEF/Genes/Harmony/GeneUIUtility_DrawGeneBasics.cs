        ...
        static IEnumerable<CodeInstruction> Transpiler(IEnumerable<CodeInstruction> instructions)
        {
            ...
            for (var i = 0; i < codes.Count; i++)
            {
                ...
                if (codes[i].opcode == OpCodes.Ldsfld && codes[i].LoadsField(loadsField))
                {
                    ...
                    yield return new CodeInstruction(OpCodes.Call, typeof(VanillaExpandedFramework_GeneUIUtility_DrawGeneBasics_Patch).GetMethod("ChooseEndogeneBackground"));

                }
                else if (codes[i].opcode == OpCodes.Ldsfld && codes[i].LoadsField(loadsFieldTwo))
                {
                    yield return new CodeInstruction(OpCodes.Ldarg_0).MoveLabelsFrom(codes[i]);
                    yield return new CodeInstruction(OpCodes.Call, typeof(VanillaExpandedFramework_GeneUIUtility_DrawGeneBasics_Patch).GetMethod("ChooseXenogeneBackground"));
                }

                else if (codes[i].opcode == OpCodes.Ldsfld && codes[i].LoadsField(loadsFieldArchite))
                {
                    yield return new CodeInstruction(OpCodes.Ldarg_0).MoveLabelsFrom(codes[i]);
                    yield return new CodeInstruction(OpCodes.Call, typeof(VanillaExpandedFramework_GeneUIUtility_DrawGeneBasics_Patch).GetMethod("ChooseArchiteBackground"));
                }
