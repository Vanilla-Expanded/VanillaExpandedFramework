...
namespace VEF.Buildings
{
    ...
    public class DoorTeleporter : ThingWithComps, IRenameable
    {
        ...
        public void RecacheBackground()
        {
            ...
            UnityEngine.Graphics.Blit(doorMaterials.backgroundTex, this.background1, Vector2.one, this.backgroundOffset, 0, 0);
            UnityEngine.Graphics.Blit(this.background1, this.background2, doorMaterials.maskMat);
        ...
        protected override void DrawAt(Vector3 drawLoc, bool flip = false)
        {
            ...
            UnityEngine.Graphics.DrawMesh(MeshPool.plane10, Matrix4x4.TRS(drawLoc, Quaternion.AngleAxis(this.rotation, Vector3.up), drawSize * 1.5f), 
                doorMaterials.MainMat, 0);
            if (this.backgroundMat != null)
                UnityEngine.Graphics.DrawMesh(MeshPool.plane10, Matrix4x4.TRS(drawLoc - Altitudes.AltIncVect / 2, Quaternion.identity, drawSize * 1.5f),
                    ...
                    if (followee != null)
                    {
                        if (followee.Map != pawn.Map)
                        {
                            ...
                            if (doorTeleportersPawn != null)
                            {
                                ...
                                {
                                    __result = JobMaker.MakeJob(VEFDefOf.VEF_UseDoorTeleporter, doorTeleportersPawn);
        ...
        public override IEnumerable<FloatMenuOption> GetFloatMenuOptions(Pawn selPawn)
        {
            ...
            else
            {
                foreach (DoorTeleporter doorTeleporter in WorldComponent_DoorTeleporterManager.Instance.DoorTeleporters.Except(this))
                {
                    ...
                    {
                        Job job = JobMaker.MakeJob(VEFDefOf.VEF_UseDoorTeleporter, this);
                        ...
                        foreach (var otherPawn in selPawn.Map.mapPawns.AllPawnsSpawned)
                        {
                            ...
                            {
                                Job job2 = JobMaker.MakeJob(VEFDefOf.VEF_UseDoorTeleporter, this);
