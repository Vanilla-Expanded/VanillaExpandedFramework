...
namespace VEF.Apparels
{
    ...
    public class CompShieldField : ThingComp, PawnGizmoProvider
    {
        ...
        private void BreakShield(DamageInfo dinfo)
        {
            ...
            if (active)
            {
                VEFDefOf.EnergyShield_Broken.PlayOneShot(new TargetInfo(HostThing));
        ...
        {
            VEFDefOf.EnergyShield_Broken.PlayOneShot(new TargetInfo(HostThing.Position, HostThing.Map));
            FleckMaker.Static(HostThing.TrueCenter(), HostThing.Map, FleckDefOf.ExplosionFlash, 12);
            for (int i = 0; i < 6; i++)
            {
