...
namespace VEF.Weapons
{
    ...
    class VanillaExpandedFramework_Building_TurretGun_IsValidTarget_Patch
{
        ...
        public static IEnumerable<CodeInstruction> Transpiler(IEnumerable<CodeInstruction> instructions)
        {
            ...
            for (int i = 0; i < codes.Count; i++)
            {
                if (codes[i].operand as MethodInfo == typeof(GenDraw).GetMethod("DrawRadiusRing", new Type[] { typeof(IntVec3), typeof(float) }) && !flag)
                {
                    ...
                    yield return new CodeInstruction(OpCodes.Call, AccessTools.Method(typeof(VanillaExpandedFramework_Building_TurretGun_DrawExtraSelectionOverlays_Transpiler), nameof(DrawConeForDirectionalTurret)));
                }
                else
                {
    ...
    public static class VanillaExpandedFramework_Building_TurretGun_DrawExtraSelectionOverlays_Patch
{
    ...
    class VanillaExpandedFramework_TurretTop_get_CurRotation_Patch
{
