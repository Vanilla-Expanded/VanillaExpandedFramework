            ...
            foreach (var (thing, conds) in from apparel in GetAllEquipment(eq)
                ...
                builder.AppendLine(
                    conds?.IsValid(req.Thing, thing.def) ?? true
                        ...
                        : $"{thing.def.LabelCap}: {req.Thing.def.LabelCap} {"VEF.IsToo".Translate()} {(req.Thing.def.techLevel > conds.techLevels.Max() ? "VEF.Advanced".Translate() : "VEF.Primitive".Translate())}");
