...
namespace VFE.Mechanoids.HarmonyPatches
{
    ...
    //TODO  - Assigned to Taranchuk, likely irrelevant due to possible VFE Mechs 2 remake
    ...
    //TODO  - Assigned to Taranchuk, code changed in origin a bit
		...
		static SimpleSidearmsPatch()
		{
			...
			if (SimpleSidearmsActive)
			{
				...
				if (type != null)
				{
                    ...
                    VEF_Mod.harmonyInstance.Patch(target, postfix: new HarmonyMethod(AccessTools.Method(typeof(SimpleSidearmsPatch), nameof(IsValidSidearmsCarrierPostfix))));
					type = AccessTools.TypeByName("SimpleSidearms.rimworld.CompSidearmMemory");
					target = AccessTools.Method(type, "GetMemoryCompForPawn");
                    VEF_Mod.harmonyInstance.Patch(target, prefix: new HarmonyMethod(AccessTools.Method(typeof(SimpleSidearmsPatch), nameof(GetMemoryCompForPawnPrefix))));
					type = AccessTools.TypeByName("SimpleSidearms.rimworld.Gizmo_SidearmsList");
					target = AccessTools.Method(type, "DrawGizmoLabel");
                    VEF_Mod.harmonyInstance.Patch(target, prefix: new HarmonyMethod(AccessTools.Method(typeof(SimpleSidearmsPatch), nameof(GizmoLabelFixer))));
				}
				else
				{
	...
	public static class FloatMenuMakerMap_ChoicesAtFor_Patch
	{
		...
		public static void AppendSidearmsOptions(Pawn pawn, ThingWithComps equipment, ref List<FloatMenuOption> __result)
		{
			try
			{
				...
				if (!PeteTimesSix.SimpleSidearms.Utilities.StatCalculator.CanPickupSidearmInstance(equipment, pawn, out string errStr))
				{
